...
class LoanApplicationAggregate:
    ...
    def __init__(self, application_id: str) -> None:
        ...
        self.decision_referred: bool = False
    ...
    def _on_FraudScreeningCompleted(self, event: StoredEvent) -> None:
        ...
        self.fraud_session_id = event.payload.get("session_id") or event.payload.get("agent_id")
    ...
    def _on_DecisionGenerated(self, event: StoredEvent) -> None:
        if self.state != ApplicationState.PENDING_DECISION:
            raise DomainError(
                f"Invalid state for DecisionGenerated: {self.state}. "
                f"Must be {ApplicationState.PENDING_DECISION}."
            )
    ...
    def _on_HumanReviewCompleted(self, event: StoredEvent) -> None:
        ...
        if override:
            ...
            self.override_reason = event.payload.get("override_reason", "")
        # Clear REFER flag — human has reviewed
        self.decision_referred = False
    ...
    def assert_no_pending_refer(self) -> None:
        """No new DecisionGenerated if a REFER is pending human review."""
        if self.decision_referred:
            raise DomainError(
                "A REFER decision is pending human review. "
                "Cannot generate another decision without completing human review."
            )
    ...
    def assert_valid_causal_chain(
        ...
        this application_id. Checks event payloads, not just stream IDs.
        ...
        for stream_id, events in session_events_map.items():
            if not events:
                raise DomainError(
                    f"Causal chain broken: session '{stream_id}' has no events"
                )

            has_decision = any(
                e.event_type in DECISION_TYPES
                and e.payload.get("application_id") == self.application_id
                for e in events
            )
            if not has_decision:
                raise DomainError(
                    f"Causal chain broken: session '{stream_id}' has no "
