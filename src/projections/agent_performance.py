...
class AgentPerformanceLedgerProjection(Projection):
    ...
    async def handle(self, event: StoredEvent) -> None:
        """Process a single event. Idempotent via UPSERT with running averages."""
        handler = getattr(self, f"_handle_{event.event_type}", None)
        if handler:
            handler(event)

    def _handle_CreditAnalysisCompleted(self, event: StoredEvent) -> None:
        ...
        if confidence is not None:
            old_avg = existing.get("avg_confidence_score") or 0.0
        ...
        if duration is not None:
            old_avg_dur = existing.get("avg_duration_ms") or 0.0
        ...
        self._store.upsert(TABLE, key, existing)

    def _handle_FraudScreeningCompleted(self, event: StoredEvent) -> None:
        payload = event.payload
        agent_id = payload.get("agent_id", "unknown")
    ...
    def _handle_DecisionGenerated(self, event: StoredEvent) -> None:
        payload = event.payload
    ...
    def _handle_HumanReviewCompleted(self, event: StoredEvent) -> None:
        payload = event.payload
        if payload.get("override"):
            # Attribute override to a generic "human_review" agent
            # In production, correlate to the original decision's agent
